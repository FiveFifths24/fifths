...
const profilesWithResolvedAvatars = await Promise.all(
  visibleSenderProfiles.map(async (profile) => ({
    ...profile,
    resolvedAvatarUrl: await signProfileMedia(
      supabase,
      profile.avatar_url,
    ),
  })),
...
{message.deleted_at ? (
  message.can_view_deleted_body && message.body ? (
    <>
      ...
      <div className="mt-3">
        <span className="inline-flex items-center rounded-full border border-amber-400/20 bg-amber-400/[0.08] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-200/70">
